...
// custom Item upload validation schema
const schema = {
  type: "object",
  properties: {
...
const useItemFilter = (data) => {
  ...
  const handleUploadItems = () => {
    if (newItems.length < 1) {
      ...
      ItemServices.addAllItems(newItems)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      setLoading(true);
      let ItemDataValidation = selectedFile.map((value) =>
        ajv.validate(schema, value)
      );

      const isBelowThreshold = (currentValue) => currentValue === true;
      const validationData = ItemDataValidation.every(isBelowThreshold);
      // console.log('validationdata',validationData)

      if (validationData) {
        ItemServices.addAllItems(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
