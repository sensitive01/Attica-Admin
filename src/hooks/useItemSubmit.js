...
const useItemSubmit = (id) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (data.originalPrice < data.price) {
        ...
        return notifyError(
          "SalePrice must be less then or equal of Item price!"
        );
      }
      ...
      if (updatedId) {
        const res = await ItemServices.updateItem(updatedId, ItemData);
        if (res) {
          if (isCombination) {
            setIsUpdate(true);
        ...
        if (isCombination) {
          ...
          setValue("unitofmeasurement", res.unitofmeasurement);
          setValue("stock", res.stock);
          setTag(JSON.parse(res.tag));
          setImageUrl(res.image);
          setVariants(res.variants);
          setValue("ItemId", res.ItemId);
      ...
      (async () => {
        try {
          ...
          if (res) {
            ...
            setValue("stock", res.stock);
            setItemId(res.ItemId ? res.ItemId : res._id);
            setBarcode(res.barcode);
            setSku(res.sku);
            setunitofmeasurement("unitofmeasurement",res.unitofmeasurement);
  ...
  //for filter related attribute and extras for every Item which need to update
  useEffect(() => {
    ...
    combo.map((com, i) => {
      if (JSON.stringify(variant).includes(JSON.stringify(com))) {
        ...
        const newCom = {
          ...
          ItemId: ItemId && ItemId + "-" + (variants.length + i),
          barcode: barcode,
          sku: sku,
          image: imageUrl[0] || "",
        };
  ...
  const handleIsCombination = () => {
    if ((isCombination && variantTitle.length) > 0) {
      swal({
        title: "Are you sure to remove combination from this Item!",
  ...
  const handleProductTap = (e, value, name) => {
    ...
    if (value) {
      if (!value)
        return notifyError(
          `${"Please save Item before adding combinations!"}`
        );
    } else {
      if (!isBasicComplete)
        return notifyError(
  ...
  const handleQuantityPrice = (value, name, id, variant) => {
    ...
    if (name === "price" && Number(variant.originalPrice) < Number(value)) {
      ...
      notifyError("SalePrice must be less then or equal of Item price!");
  ...
  //for change language in Item drawer
  ...
  //for handle Item slug
  const handleItemSlug = (value) => {
